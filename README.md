...
There is a spare 5V output on the board. This can be used to power a display, and has been tested with the official Raspberry Pi Touch Display. If you intend to draw more than 1.5A continuously (including powering the Raspberry Pi), or placing the CarPiHAT in an area that is likely to suffer from high ambient temperatures, it is strongly recomended to include a fan to cool the power supply circuitry.
